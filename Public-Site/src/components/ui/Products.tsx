...
export function Products() {
  ...
  return <section id="products" className="py-12 md:py-16 lg:py-20 bg-gradient-to-b from-gray-50 to-white overflow-hidden">
    <div className="container mx-auto px-4 md:px-6 lg:px-8">
      ...
      <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
        ...
        <motion.p initial={{
          ...
          Discover our complete range of quality detergents and skin care
          products, made with love for your family.
        </motion.p>
      </div>
      {featuredProduct && <div className="bg-[#0A4D68] rounded-2xl md:rounded-3xl p-6 md:p-10 lg:p-12 mb-12 md:mb-16 text-white">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 items-center">
          <motion.div initial={{
            ...
            <div className="relative z-10  rounded-2xl md:rounded-3xl p-6 md:p-8 ">
              <div className="aspect-[4/3] bg-gray-100 rounded-lg md:rounded-xl overflow-hidden flex items-center justify-center relative">
                ...
                />
                <div className="absolute inset-0 bg-gradient-to-br from-blue-100 to-white opacity-50 hidden" style={{display: 'none'}}>
                  <div className="text-center p-4 md:p-8 relative z-10 flex flex-col items-center justify-center h-full">
                    <div className="w-32 h-24 sm:w-40 sm:h-28 md:w-48 md:h-32 bg-[#088395] rounded-lg shadow-lg mx-auto mb-3 md:mb-4 flex items-center justify-center">
                      <featuredProduct.icon size={48} className="text-white" />
                    </div>
          ...
          <motion.div initial={{
            ...
            <div className="flex flex-wrap gap-2 mb-4 md:mb-6">
              {featuredProduct.sizes.map(size => <span key={size} className="px-3 py-1 bg-white/10 rounded-full text-xs sm:text-sm font-medium">
                      {size}
                    </span>)}
            </div>
            <ul className="space-y-3 mb-6 md:mb-8">
              {featuredProduct.features.map((feature, i) => <motion.li key={i} initial={{
                opacity: 0,
                x: 20
              }} whileInView={{
                opacity: 1,
            ...
            <Button variant="white" size="lg" onClick={() => document.getElementById('contact')?.scrollIntoView({
              behavior: 'smooth'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {otherProducts.map((product, index) => <motion.div key={product.name} initial={{
          opacity: 0,
          y: 20
        }} whileInView={{
          opacity: 1,
          ...
          delay: index * 0.1
        }} className="bg-white rounded-xl md:rounded-2xl shadow-lg border border-gray-100 overflow-hidden group hover:shadow-xl transition-shadow duration-300">
          {/* Product Header */}
          <div className={`${product.color} p-6 md:p-8 text-white relative overflow-hidden`}>
            <div className="absolute top-0 right-0 w-32 h-32 bg-white/10 rounded-full -translate-y-1/2 translate-x-1/2" />
            <product.icon size={40} className="mb-4 relative z-10" />
            <h3 className="text-lg md:text-xl font-bold mb-2 relative z-10">
              {product.name}
            </h3>
            <div className="flex flex-wrap gap-1.5 relative z-10">
              {product.sizes.map(size => <span key={size} className="px-2 py-0.5 bg-white/20 rounded text-xs font-medium">
                      {size}
                    </span>)}
            </div>
          </div>
          <div className="p-6">
            <ul className="space-y-2.5 mb-6">
              {product.features.map((feature, i) => <li key={i} className="flex items-start gap-2 text-sm text-gray-600">
